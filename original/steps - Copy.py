...
async def vet_question_loop( # NOTE adding the pipelinestep class would make this a bit more complex, rather than less; so this is not refactored to use that class
    ...
    try:
        ...
        question_checker = QuestionCheckGenerationStep( #changed for debugging 2025-02-09 MFJ
...
#added for debugging of phase 2 2025-02-09 MFJ
class QuestionCheckGenerationStep(GenerationStep):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.debug_log_path = os.path.join(OUTPUT_DIR, "debug_tracking")
        os.makedirs(self.debug_log_path, exist_ok=True)
        self.debug_file = os.path.join(self.debug_log_path, "phase2_question_check_debug.jsonl")

    def log_check_debug(self, validation_id, paragraph_idx, question_idx, success, response):
        debug_entry = {
            "timestamp": datetime.now().isoformat(),
    ...
    async def generate(self, paragraph_idx=None, question_idx=None, **kwargs):
        ...
        self.log_check_debug(validation_id, paragraph_idx, question_idx, success, full_output)
        return result, full_output

#added for debugging of phase 3 2025-02-11 MFJ
class ContextRepairDebugStep(GenerationStep):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.debug_log_path = os.path.join(OUTPUT_DIR, "debug_tracking")
        os.makedirs(self.debug_log_path, exist_ok=True)
